...
params.seqtype = 'dna'
params.solver = 'glpk'
params.enumerations = 1
params.beta = 0.009
//use this to combine accordingly for dna/rna samples
// params.base_index_path = "$baseDir/data/indices/yara" 
base_index_path = ''
base_index_path = Channel.fromPath(params.base_index_path)
params.base_index_name = ''
additional_config_files = ''
additional_config_files = Channel.fromPath(params.additional_config_files)
additional_config_files = "${workflow.launchDir}/${additional_config_files}"
// params.base_index_path = additional_config_files
...
docker_container = 'nfcore/hlatyping:1.2.0'
// docker.enabled = true
// Avoid this error:
...
// Stage config files
ch_multiqc_config = file("${params.additional_config_files}/assets/multiqc_config.yaml", checkIfExists: true)
ch_multiqc_custom_config = params.multiqc_config ? Channel.fromPath(params.multiqc_config, checkIfExists: true) : Channel.empty()
ch_output_docs = file("${params.additional_config_files}/docs/output.md", checkIfExists: true)
ch_output_docs_images = file("${params.additional_config_files}/docs/images/", checkIfExists: true)
...
summary['Index Location']   = "$workflow.launchDir/$params.base_index_path/$base_index_name"
...
params.base_index_path = "$workflow.launchDir/$params.base_index_path/$base_index_name"
base_index_path = "$workflow.launchDir/$params.base_index_path/$base_index_name"
...
if ( !params.bam  ) { // FASTQ files processing
    process unzip {
        ...
        pod annotation: 'scheduler.illumina.com/presetSize', value: 'standardhicpu-9xlarge'
        input:
    ...
    process remap_to_hla {
        ...
        pod annotation: 'scheduler.illumina.com/presetSize', value: 'standardhicpu-9xlarge'
        input:
        path(data_index) from base_index_path
        ...
        script:
        def full_index = "${workflow.launchDir/params.base_index_path}/$base_index_name"
        if (params.single_end)
            """
...
process make_ot_config {
    ...
    script:
    """
    python ${workflow.launchDir/params.additional_config_files}/bin/configbuilder --max-cpus ${params.max_cpus} --solver ${params.solver} > config.ini
    """
}
...
process run_optitype {
    publishDir "${params.outdir}/optitype/", mode: params.publish_dir_mode, overwrite: false
    container docker_container
    pod annotation: 'scheduler.illumina.com/presetSize', value: 'standardhicpu-9xlarge'
    ...
    env MPLCONFIGDIR from mplconfigdir
    
    output:
    file "${pattern}*" 
    file("${pattern}/*tsv") into optitype_output
...
process output_documentation {
    ...
    script:
    """
    python ${workflow.launchDir/params.additional_config_files}/bin/markdown_to_html.py $output_docs -o results_description.html
    """
}
...
process get_software_versions {
    ...
    python ${workflow.launchDir/params.additional_config_files}/bin/scrape_software_versions.py &> software_versions_mqc.yaml
    """
}
...
process copy_workfiles {
    ...
    input:
        file(optitype_result) from optitype_output
...
workflow.onComplete {
    ...
    def tf = new File("${workflow.launchDir}/params.additional_config_files/assets/email_template.txt")
    def txt_template = engine.createTemplate(tf).make(email_fields)
    def email_txt = txt_template.toString()

    // Render the HTML template
    def hf = new File("${workflow.launchDir}/params.additional_config_files/assets/email_template.html")
    def html_template = engine.createTemplate(hf).make(email_fields)
    def email_html = html_template.toString()

    // Render the sendmail template
    def smail_fields = [ email: email_address, subject: subject, email_txt: email_txt, email_html: email_html, baseDir: "$baseDir", mqcFile: mqc_report, mqcMaxSize: params.max_multiqc_email_size.toBytes() ]
    def sf = new File("${workflow.launchDir}/params.additional_config_files/assets/sendmail_template.txt")
