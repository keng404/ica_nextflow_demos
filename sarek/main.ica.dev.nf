...
params.genomes_base = null
params.save_reference = null
params.help = false
params.no_intervals = null
params.nucleotides_per_second = 1000
params.sentieon = null
params.skip_qc = null
params.target_bed = false
params.tools = null
params.trim_fastq = false
params.clip_r1 = 0
params.clip_r2 = 0
...
params.split_fastq = null
params.aligner = 'bwa-mem'
params.markdup_java_options = '-Xms4000m'
params.use_gatk_spark = false
params.save_bam_mapped = true
params.skip_markduplicates = null
params.ascat_ploidy = null
params.ascat_purity = null
params.cf_coeff = 0.05
params.cf_contamination = null
params.cf_contamination_adjustment = null
params.cf_ploidy = 2
params.cf_window = null
params.generate_gvcf = null
params.no_strelka_bp = null
params.pon = false
params.pon_index = false
params.ignore_soft_clipped_bases = null
params.umi = null
params.read_structure1 = null
params.read_structure2 = null
params.annotate_tools = null
...
params.genomes['GRCh38'].dbsnp = "${params.igenomes_base}/Homo_sapiens/GATK/GRCh38/Annotation/GATKBundle/dbsnp_146.hg38.vcf.gz"
...
process UMIMapBamFile {
    ...
    ${aligner} mem -p -t 8 -C -M -R \"@RG\\tID:${idSample}\\tSM:${idSample}\\tPL:Illumina\" \
    ${fasta} - | \
...
process MapReads {
    ...
    extra = status == 1 ? "-B 3" : ""
    convertToFastq = hasExtension(inputFile1, "bam") ? "gatk --java-options -Xmx32g SamToFastq --INPUT=${inputFile1} --FASTQ=/dev/stdout --INTERLEAVE=true --NON_PF=true | \\" : ""
    input = hasExtension(inputFile1, "bam") ? "-p /dev/stdin - 2> >(tee ${inputFile1}.bwa.stderr.log >&2)" : "${inputFile1} ${inputFile2}"
    aligner = params.aligner == "bwa-mem2" ? "bwa-mem2" : "bwa"
    """
    ${convertToFastq}
    ${aligner} mem -K 100000000 -R \"${readGroup}\" ${extra} -t 8 -M ${fasta} \
    ${input} | \
    samtools sort --threads 8 -m 2G - > ${idSample}_${idRun}.bam
    """
}
...
process Sentieon_MapReads {
    ...
    extra = status == 1 ? "-B 3" : ""
    """
    sentieon bwa mem -K 100000000 -R \"${readGroup}\" ${extra} -t 8 -M ${fasta} \
    ${inputFile1} ${inputFile2} | \
    sentieon util sort -r ${fasta} -o ${idSample}_${idRun}.bam -t 8 --sam2bam -i -
    """
}
...
process MergeBamMapped {
    ...
    script:
    """
    samtools merge --threads  8 ${idSample}.bam ${bam}
    """
}
...
process MarkDuplicates {
    ...
    script:
    markdup_java_options = 32 > 8 ? params.markdup_java_options : "\"-Xms" +  (32/ 2).trunc() + "g -Xmx" + (32- 1) + "g\""
...
process MergeBamRecal {
    ...
    script:
    """
    samtools merge --threads 8 ${idSample}.recal.bam ${bam}
...
process HaplotypeCaller {
    ...
    """
    gatk --java-options "-Xmx32g -Xms6000m -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10" \
        HaplotypeCaller \
        -R ${fasta} \
        -I ${bam} \
        ${intervalsOptions} \
...
process StrelkaSingle {
    ...
    script:
    beforeScript = params.target_bed ? "bgzip --threads 8 -c ${targetBED} > call_targets.bed.gz ; tabix call_targets.bed.gz" : ""
    ...
    configureStrelkaGermlineWorkflow.py \
        ...
        ${options} \
        --runDir Strelka
    python Strelka/runWorkflow.py -m local -j 8
...
process MantaSingle {
    ...
    configManta.py \
        ...
        ${options} \
        --runDir Manta
    python Manta/runWorkflow.py -m local -j 8 
...
process TIDDIT {
    ...
    bgzip --threads 8 -c TIDDIT_${idSample}.vcf > TIDDIT_${idSample}.vcf.gz
...
process ConcatVCF {
    ...
    """
    concatenateVCFs.sh -i ${fastaFai} -c 8 -o ${outputFile} ${options} ${intervalsOptions}
    """
}
...
process Snpeff {
    ...
    publishDir "${params.outdir}/Reports/${idSample}/snpEff", mode:params.publish_dir_mode
...
process VEP {
    tag "${idSample} - ${variantCaller} - ${vcf}"
    publishDir "${params.outdir}/Reports/${idSample}/VEP", mode: params.publish_dir_mode
