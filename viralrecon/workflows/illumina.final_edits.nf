...
if (!params.skip_fastp) {
    process.ext.args = '--cut_front--cut_tail--trim_poly_x--cut_mean_quality30--qualified_quality_phred30--unqualified_percent_limit10--length_required50'
}
    FASTQC_FASTP (
        ch_cat_fastq,
...
if (!params.skip_multiqc) {
    ...
    publishDir =  path: { "${params.outdir}/multiqc/${params.artic_minion_caller}" }, mode: params.publish_dir_mode, saveAs: { filename -> filename.equals('versions.yml') ? null : filename }
}
...
if (!params.skip_picard_metrics) {
    process.ext.args = '--VALIDATION_STRINGENCYLENIENT--TMP_DIRtmp'
}
...
if (!params.skip_mosdepth) {
    process.ext.args = '--fast-mode--by200'
}
        MOSDEPTH_GENOME (
            ch_bam.join(ch_bai, by: [0]),
            [],
...
if (!params.skip_mosdepth) {
    process.ext.args = '--fast-mode--use-median--thresholds0,1,10,50,100,500'
}
            MOSDEPTH_AMPLICON (
                ch_bam.join(ch_bai, by: [0]),
