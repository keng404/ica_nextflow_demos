...
publishDir  path: { "${params.outdir}/illumina"}, mode: "copy", saveAs: { filename -> filename.equals('versions.yml') ? null : filename }
...
println("Variant caller is " + variant_caller)
println("Assembler is " + assemblers)
process = [:]
process.ext = [:]
/*
========================================================================================
...
//
include { CUTADAPT } from ' ../modules/local/cutadapt.dev.nf '
include { MULTIQC  } from ' ../modules/local/multiqc_illumina.dev.nf '
include { PLOT_MOSDEPTH_REGIONS as PLOT_MOSDEPTH_REGIONS_GENOME   } from ' ../modules/local/plot_mosdepth_regions.dev.nf '
include { PLOT_MOSDEPTH_REGIONS as PLOT_MOSDEPTH_REGIONS_AMPLICON } from ' ../modules/local/plot_mosdepth_regions.dev.nf '
include { MULTIQC_TSV_FROM_LIST as MULTIQC_TSV_FAIL_READS         } from ' ../modules/local/multiqc_tsv_from_list.dev.nf '
include { MULTIQC_TSV_FROM_LIST as MULTIQC_TSV_FAIL_MAPPED        } from ' ../modules/local/multiqc_tsv_from_list.dev.nf '
include { MULTIQC_TSV_FROM_LIST as MULTIQC_TSV_NEXTCLADE          } from ' ../modules/local/multiqc_tsv_from_list.dev.nf '
//
// SUBWORKFLOW: Consisting of a mix of local and nf-core/modules
//
include { INPUT_CHECK         } from ' ../subworkflows/local/input_check.dev.nf '
include { PREPARE_GENOME      } from ' ../subworkflows/local/prepare_genome_illumina.dev.nf '
include { VARIANTS_IVAR       } from ' ../subworkflows/local/variants_ivar.dev.nf '
include { VARIANTS_BCFTOOLS   } from ' ../subworkflows/local/variants_bcftools.dev.nf '
include { CONSENSUS_IVAR      } from ' ../subworkflows/local/consensus_ivar.dev.nf '
include { CONSENSUS_BCFTOOLS  } from ' ../subworkflows/local/consensus_bcftools.dev.nf '
include { VARIANTS_LONG_TABLE } from ' ../subworkflows/local/variants_long_table.dev.nf '
include { ASSEMBLY_SPADES     } from ' ../subworkflows/local/assembly_spades.dev.nf '
include { ASSEMBLY_UNICYCLER  } from ' ../subworkflows/local/assembly_unicycler.dev.nf '
include { ASSEMBLY_MINIA      } from ' ../subworkflows/local/assembly_minia.dev.nf '
/*
========================================================================================
...
//
include { CAT_FASTQ                     } from ' ../modules/nf-core/modules/cat/fastq/main.dev.nf '
include { FASTQC                        } from ' ../modules/nf-core/modules/fastqc/main.dev.nf '
include { KRAKEN2_KRAKEN2               } from ' ../modules/nf-core/modules/kraken2/kraken2/main.dev.nf '
include { PICARD_COLLECTMULTIPLEMETRICS } from ' ../modules/nf-core/modules/picard/collectmultiplemetrics/main.dev.nf '
include { CUSTOM_DUMPSOFTWAREVERSIONS   } from ' ../modules/nf-core/modules/custom/dumpsoftwareversions/main.dev.nf '
include { MOSDEPTH as MOSDEPTH_GENOME   } from ' ../modules/nf-core/modules/mosdepth/main.dev.nf '
include { MOSDEPTH as MOSDEPTH_AMPLICON } from ' ../modules/nf-core/modules/mosdepth/main.dev.nf '
//
// SUBWORKFLOW: Consisting entirely of nf-core/modules
//
include { FASTQC_FASTP           } from ' ../subworkflows/nf-core/fastqc_fastp.dev.nf '
include { ALIGN_BOWTIE2          } from ' ../subworkflows/nf-core/align_bowtie2.dev.nf '
include { PRIMER_TRIM_IVAR       } from ' ../subworkflows/nf-core/primer_trim_ivar.dev.nf '
include { MARK_DUPLICATES_PICARD } from ' ../subworkflows/nf-core/mark_duplicates_picard.dev.nf '
/*
========================================================================================
    RUN MAIN WORKFLOW
========================================================================================
...
if (!params.skip_fastp) {
    process.ext.args = '--cut_front --cut_tail --trim_poly_x --cut_mean_quality 30 --qualified_quality_phred 30 --unqualified_percent_limit 10 --length_required 50'
}
    FASTQC_FASTP (
        ch_cat_fastq,
...
if (!params.skip_kraken2) {
    process.ext.args = '--report-zero-counts'
}
        KRAKEN2_KRAKEN2 (
            ch_variants_fastq,
...
if (!params.skip_multiqc) {
    process.ext.args = [ '-k yaml', params.multiqc_title ? "--title \"$params.multiqc_title\"" : '' ].join(' ').trim()
}
...
if (!params.skip_picard_metrics) {
    process.ext.args = '--VALIDATION_STRINGENCY=LENIENT --TMP_DIR=tmp'
}
...
if (!params.skip_mosdepth) {
    process.ext.args = '--fast-mode --by 200'
}
        MOSDEPTH_GENOME (
            ch_bam.join(ch_bai, by: [0]),
            [],
...
if (!params.skip_mosdepth) {
    process.ext.args = '--input_suffix .regions.bed.gz'
}
...
if (!params.skip_mosdepth) {
    process.ext.args = '--fast-mode --use-median--thresholds 0,1,10,50,100,500'
}
            MOSDEPTH_AMPLICON (
                ch_bam.join(ch_bai, by: [0]),
...
if (params.protocol == 'amplicon' && !params.skip_cutadapt) {
    process.ext.args = '--overlap5--minimum-length30--error-rate0.1'
}
        CUTADAPT (
            ch_assembly_fastq,
...
if ('spades' in assemblers) {
    process.ext.args = params.spades_mode?"--$params.spades_mode":''
}
        ASSEMBLY_SPADES (
...
if ('minia' in assemblers) {
    process.ext.args = '-kmer-size31-abundance-min20'
}
        ASSEMBLY_MINIA (
            ch_assembly_fastq,
            PREPARE_GENOME.out.fasta,
