process MOSDEPTH {
	publishDir  path: { "${params.outdir}/mosdepth"}, mode: "copy", saveAs: { filename -> filename.equals('versions.yml') ? null : filename }
    tag "$meta.id"
...
memory '48 GB'
    ...
    script:
    def args = task.ext.args ?: '--fast-mode --use-median --thresholds 0,1,10,50,100,500'
    def prefix = task.ext.prefix ?: "${meta.id}"
    def reference = fasta ? "--fasta ${fasta}" : ""
    def interval = bed ? "--by ${bed}" : "--by 1000"
    ...
    if (!bed && args.contains("--thresholds") && !interval.contains("--by")) {
