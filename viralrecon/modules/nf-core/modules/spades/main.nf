process SPADES {
    tag "$meta.id"
    label 'process_high'
    publishDir  path: { "${params.outdir}/spades"}, mode: "copy", saveAs: { filename -> filename.equals('versions.yml') ? null : filename }
    ...
    script:
    def args = params.spades_mode ? "--${params.spades_mode}" : ''
