process PLASMIDID {
    publishDir  path: { "${params.outdir}/plasmidid"}, mode: "copy", saveAs: { filename -> filename.equals('versions.yml') ? null : filename }
    ...
    script:
    def args = task.ext.args   ?: '--only-reconstruct -C 47 -S 47 -i 60 --no-trim -k 0.80'
