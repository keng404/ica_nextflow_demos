...
println("Reiterate KRAKEN2 DB is here: " +  params.kraken2_db)
...
if (!params.skip_variants) {
    ...
    if (!params.skip_assembly) {
        if (!params.skip_blast) {
            if (params.blast_db) {
                if (params.blast_db.endsWith('.tar.gz') || params.blast_db.endsWith('.tgz') ) {
                    UNTAR_BLAST_DB (
                        [ [:], params.blast_db ]
                    )
                ...
                process.ext.args = '-parse_seqids -dbtype nucl'
                BLAST_MAKEBLASTDB (
                    ch_fasta
                )
