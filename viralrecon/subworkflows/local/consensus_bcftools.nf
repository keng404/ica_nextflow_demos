...
if (!params.skip_consensus && params.consensus_caller == 'bcftools') {
    process.ext.args = [ '--output-type z', variant_caller == 'ivar' ? "--include 'FORMAT/ALT_FREQ >= 0.75'" : '', variant_caller == 'bcftools' ? "--include 'FORMAT/AD[:1] / FORMAT/DP >= 0.75'" : '', ].join(' ').trim()
    process.ext.prefix =  "filtered" 
}
    BCFTOOLS_FILTER (
        vcf
    )
