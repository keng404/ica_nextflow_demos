process IQTREE {
    ...
    time '1day'
    publishDir  enabled: true,mode: "${params.publish_dir_mode}",saveAs: { filename -> filename.endsWith(".treefile") ? "iqtree_phylogeny.nh" : filename  },path: { "${params.outdir}/combined/phylogeny/iqtree" },pattern: "*"
    input:
    path alignment
    val constant_sites
    output:
    ...
    script:
    def args = "-alrt 1000 -B 1000 -m MFP -czb"
