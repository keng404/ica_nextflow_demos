process GATK4_LOCALCOMBINEGVCFS {
    ...
    time '1day'
    publishDir  enabled: true,mode: "${params.publish_dir_mode}",path: { "${params.outdir}/combined/gvcf"},pattern: "*{vcf.gz,vcf.gz.tbi}"
    input:
    val meta
    path vcf
    path fasta
    ...
    def skip_samples      = "${params.skip_samples}"
    def skip_samples_file = "${params.skip_samples_file}"
